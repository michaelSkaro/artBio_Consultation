...
# substring 1:15 in sample ID column
# select only the DE genes 
# why are there no normal expressions in this analysis? Maybe TCGA biolinks or UCSC has specifically the normal data?


generate.a.gene.expression.matrix <- function(proj, val){
...
```{r Build functions for Heatmaps for DE genes}
# In this section we will use the list of differentially expressed genes from 
# each of the indications analyzed to produce a Heatmap for each of the projects
# Then we will also create a unified Heat map where we make a single plot with each
# of the projects as a column and each DE gene from any project as a row.
# Finally, we will take the the top30 genes from each of the indications and 
# the annotated genes from the GESP paper. Intersect them to make a Venn diagram and 
# make a HM of each of these genes using the CPMs or TPMs? 

create.heatmap.from.DE.genes <- function(gene.list, gene.expression.matrix){
